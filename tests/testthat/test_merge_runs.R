...
childFeatures <- function(){
  df <- data.frame(transition_group_id = 4618L,
                   ...
                   intensity = c(229.707813, 0.9945442, 29.670797, 2.622945, 4.2806256, 24.7804246),
...
test_that("test_getNodeRun",{
  ...
  expect_equal(outData[[2]], masterXICs_DIAlignR[[2]], tolerance = 1e-04)
...
test_that("test_getChildFeature",{
  ...
  outData <- getChildFeature(newXICs[[1]], newXICs[[2]], df.ref, df.eXp, params)
...
test_that("test_trfrParentFeature",{
  ...
  df[,3:6] <- list(c(5237.8, 5393.2, 5514.4, 4997.2, 5026.2),
                   c(229.707813, 29.670797, 2.622945, 24.780425, 15.81119),
...
test_that("test_getChildXICs",{
  ...
  expect_identical(dim(outData[[2]][[1]]), c(184L, 5L))
  expect_identical(colnames(outData[[2]][[1]]), c("indexAligned.ref", "indexAligned.eXp",
              "tAligned.ref", "tAligned.eXp", "alignedChildTime"))
  alignObj@indexA_aligned[alignObj@indexA_aligned == 0L] <- NA_integer_
  expect_identical(alignObj@indexA_aligned, outData[[2]][[1]][,"indexAligned.ref"])
  alignObj@indexB_aligned[alignObj@indexB_aligned == 0L] <- NA_integer_
  expect_identical(alignObj@indexB_aligned, outData[[2]][[1]][,"indexAligned.eXp"])
  expect_equal(outData[[3]], list(ab = 36.8015, ba = 38.6594), tolerance = 1e-04)
})
